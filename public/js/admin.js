...
(function () {
  ...
  // قراءة استجابة آمنة: لو الخادم/الاستضافة ردوا صفحة HTML بنحوّلها لرسالة واضحة
  ...
  function doPublish() {
    ...
    selectedVideos.forEach(function (f) { fd.append('video', f, f.name); });
  ...
  if ($('img-picker')) $('img-picker').addEventListener('click', function () { $('pimages').click(); });
  $('pimages').addEventListener('change', function () {
    const files = Array.prototype.slice.call(this.files || []);
    files.forEach(function (f) { selectedImages.push(f); });
    this.value = '';
    renderImgPreviews();
  });
  ...
  $('pvideo').addEventListener('change', function () {
    const files = Array.prototype.slice.call(this.files || []);
    this.value = '';
    files.forEach(function (f) { selectedVideos.push(f); });
    renderVidPreviews();
  });
  function renderVidPreviews() {
    ...
    selectedVideos.forEach(function (f, i) {
      ...
      d.innerHTML = '<video src="' + url + '" muted></video><span class="vidtag">فيديو</span><button class="rm" type="button">&times;</button>';
      d.querySelector('.rm').addEventListener('click', function () { selectedVideos.splice(i, 1); renderVidPreviews(); });
      box.appendChild(d);
    });
  }
